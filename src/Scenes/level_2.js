class Level_2 extends Phaser.Scene {
    ...
    create() {
            ...
            // Make it collidable
            this.groundsLayer.setCollisionByProperty({ collides: true });
            this.lavaNspikesLayer.setCollisionByProperty({ collides: true });
            ...
            // Flag overlap: gate by coin count
            this.physics.add.overlap(this.player, this.flagGroup, () => {
                if (this.collectedCoins >= this.totalCoins) {
                    this.scene.start("endScene");
                } else {
                    const msg = this.add.text(this.player.x - 50, this.player.y - 50, "Collect all coins!", {
                        fontSize: '12px',
                        fill: '#ff0000'
